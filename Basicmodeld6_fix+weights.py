...
def symbolic_revenue(beta, w):
    ...
    for s in range(S):
        E_s = sp.Matrix(Es[s])
        cost0=E_s.dot(lambda0)
        cost1=f1+E_s.dot(lambda1)
        cost2=f2+E_s.dot(lambda2)

        costs = [cost0, cost1, cost2] #cost for the customer
        ...
        total += w[s]*sum(costs[i] * probs[i] for i in range(1, N+1))
    return total
...
revenue_sym = symbolic_revenue(beta, w_basic)
U_sym = -revenue_sym  
