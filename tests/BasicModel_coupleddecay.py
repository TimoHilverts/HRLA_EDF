...
# If your library allows a simple function, we can use this "Hybrid" approach:
def make_coupled_h(h0, a_min, a_max, K):
    """
    h_k = h0 * a_min / a_k
    
    Keeps h large when a is small (exploration)
    and small when a is large (exploitation).
    h * a_k = h0 * a_min = constant throughout run.
    """
    def coupled_h(k, K):
        a_k = a_min + (a_max - a_min) * (k / max(K, 1)) ** 2
        a_k = max(a_k, a_min)
        return h0 * a_min / a_k
    return coupled_h
...
if __name__ == "__main__":
    ...
    # Run for each a_max separately since make_coupled_h depends on a_max
    for a_max in [0.1, 0.5, 1, 5]:
        algorithm = GO.HRLA(
            d=d, M=M, N=1, K=K,
            h=make_coupled_h(h0=1e-4, a_min=a_min, a_max=a_max, K=K),
            a_min=a_min,
            title=f"{title}_amax{a_max}",
            U=U_z, dU=dU_z, initial=initial,
        )
        samples_filename = algorithm.generate_samples(
            As=[a_max], sim_annealing=True
        )
