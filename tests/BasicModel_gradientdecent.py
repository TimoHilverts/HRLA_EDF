...
def poly_schedule(k, K, h0=1e-4, alpha=0.55):
    # Decays as k^{-alpha}, alpha in (0.5, 1) is the standard range
    # alpha=0.55 balances exploration vs convergence
    return h0 / (1 + k) ** alpha
...
if __name__ == "__main__":
    ...
    samples_filename = algorithm.generate_samples(As=[0.1, 0.3, 0.5, 1, 3], sim_annealing=True)
    ...
    best_z, best_x, best_revenue, best_k = postprocessor.get_best_overall(
        a_idx=0, dpi=1, to_x=to_x
    )
    print(f"\nHRLA best revenue: {best_revenue:.4f}")
    print(f"HRLA best z:       {best_z}")
    print(f"HRLA best x:       {best_x}")
    print(f"Found at k:        {best_k}")

    # ============================================================
    # Gradient descent refinement from HRLA solution
    # ============================================================
    ...
    best_z_refined = result.x
    best_x_refined = to_x(best_z_refined)
    best_revenue_refined = -result.fun

    print(f"\nAfter gradient descent refinement:")
    print(f"  Revenue:  {best_revenue_refined:.4f}")
    print(f"  Best z:   {best_z_refined}")
    print(f"  Best x:   {best_x_refined}")
    print(f"  GD iters: {result.nit}")
    print(f"  Basin:    {'GLOBAL' if best_revenue_refined > 1514 else 'LOCAL'}")
    print(f"  Gap closed: {best_revenue_refined - best_revenue:.4f}")