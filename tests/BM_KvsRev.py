...
def initial() -> np.ndarray:
    ...
    z0 += np.random.normal(0.0, , size=d)
    return z0
...
if __name__ == "__main__":
    ...
    algorithm = GO.HRLA(
        d=d, M=100, N=1, K=100000, h=0.000001,
        title=title, U=U_z, dU=dU_z, initial=initial,
    )

    samples_filename = algorithm.generate_samples(As=[1, 5, 10, 15, 20], sim_annealing=True)
    postprocessor = PostProcessor(samples_filename)
    Ks, revenues, best_x, best_revenue = postprocessor.get_running_best_curve(a_idx=2, dpi=1)
    ...
    postprocessor.plot_running_best_revenue(a_idx=2, dpi=1)