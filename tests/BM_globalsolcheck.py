...
# Evaluate gradient at several local optima vs global optimum
#Scan the objective along z[2] direction from local to global optimum
z_local = np.array([2.429, 1.007, 1.911])
z_global = np.array([1.127, 0.977, 2.428])

print("Scanning path from local to global optimum:")
for t in np.linspace(0, 1, 20):
    z_path = (1-t) * z_local + t * z_global
    print(f"  t={t:.2f} | z={z_path.round(3)} | revenue={-U_z(z_path):.4f}")