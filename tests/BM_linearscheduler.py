...
def make_coupled_h(h0, a_min, a_max, K):
    def coupled_h(k, K):
        a_k = a_min + (a_max - a_min) * (
            np.log(1 + k) / np.log(1 + K)
        )
        return h0 * (a_min / a_k)
    return coupled_h
...
if __name__ == "__main__":
    ...
    samples_filename = algorithm.generate_samples(As=[0.01, 0.1, 1, 5, 10, 15, 20, 50, 100], sim_annealing=True)
    ...
    # print("best z:", best_z)
    # print("best x:", best_x)
    # print("found at iteration K =", best_k)