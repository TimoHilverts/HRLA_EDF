...
# ============================================================
# MODEL (NumPy exact gradient) — SIMPLIFIED:
# ✅ One competitor (2 contracts) + FIXED FEE
# ✅ No reservation bills / No disutilities
# ✅ Retailer: VARIABLE ONLY (no fixed fee)
# ============================================================
...
# ============================================================
# ONE competitor (2 contracts) — variable prices + fixed fee
# ============================================================
comp_tou = np.array([0.355, 0.250], dtype=float)   # C1: (day, night)
comp_flat = 0.342                                  # C2: flat
comp_fixed = 196.0                                 # <--- NECESSARY CHANGE
...
def profit_and_grad(x: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    for s in range(S):
        ...
        # Competitor bills now include fixed fee
        b_c1 = comp_tou[0] * E_day + comp_tou[1] * E_night + comp_fixed
        b_c2 = comp_flat * T + comp_fixed
        # Retailer bills remain variable only
        b_r1 = p * T
        ...
        def dr_direction(db_dtheta: np.ndarray) -> float:
            ...
            dP = P * (du - du_bar)
            return (db_dtheta[2] * P[2] + b_r1 * dP[2] +
...
title = "BasicModel_1comp_FixedFee_numpy"
d = 3
...
if __name__ == "__main__":
    ...
    algorithm = GO.HRLA(
        d=d, M=100, N=1, K=20000, h=0.0001,
