...
def decaying_h(k, K):
    start_h = 1e-3
    end_h = 1e-6
    return start_h + (end_h - start_h) * (k / K)
if __name__ == "__main__":

    # Sweep to find best alpha_decay
    for alpha in [0.25]:
        algorithm = GO.HRLA(
            d=d, M=100, N=1, K=10000,
            h=make_slow_poly_h(1e-4, alpha),
            a_min=0.0001,
            title=f"supervisor_{alpha}",
            U=U_z, dU=dU_z, initial=initial,
        )
        samples_filename = algorithm.generate_samples(
            As=[0.1, 1, 5, 10, 20], sim_annealing=True
        )
