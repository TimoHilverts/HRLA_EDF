...
def make_piecewise_h(h_high, h_low, split, K):
    """
    Phase 1 (first split fraction): h_high for valley crossing
    Phase 2 (remaining fraction):   h_low for precise basin exploration
    
    h_high = 1e-4  — large steps for crossing valley of width 1.37
    h_low  = 1e-7  — small steps for exploring basin of width 0.001
    split  = 0.7   — 70% crossing, 30% exploitation
    """
    def piecewise_h(k, K):
        if k < int(split * K):
            return h_high
...
if __name__ == "__main__":
    ...
    samples_filename = algorithm.generate_samples(As=[0.1, 0.5, 1, 5, 10, 20], sim_annealing=True)
