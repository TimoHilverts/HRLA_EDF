...
if __name__ == "__main__":
    ...
    for a_val in [2, 4, 6, 8, 10]:
        algorithm = GO.HRLA(
            d=d, M=100, N=1, K=10000,
            h=make_constant_h(1e-3),
            title=f"a_search_{a_val}",
            U=U_z, dU=dU_z, initial=initial,
        )
        samples_filename = algorithm.generate_samples(As=[a_val], sim_annealing=False)
        ...
        print(f"a={a_val} | best revenue: {best_revenue:.4f} | best z: {best_z} | found at k={best_k}")