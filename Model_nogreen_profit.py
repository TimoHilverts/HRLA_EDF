...
shift=0.25
Es=np.array([shifting_cons(E, shift) for E in Es_original]) #the shifted consumption matrix Es
...
profit_sym = symbolic_profit(beta, w_basic) #here we now include w_basic=[1, 1, 1] to check the basic case
U_sym = -profit_sym  
...
algorithm = GO.HRLA(d=d, M=1, N=1, K=15000, h=0.01, title=title, U=U, dU=dU, initial=initial)
samples_filename_HRLA = algorithm.generate_samples(As=[3], sim_annealing=True)
...
postprocessor.compute_tables([10, 100, 3000, 15000], 1, "best") #This provides the tables with a and K vals, together with optimal revenue
...
#the optimal revenue.
bests = postprocessor.get_best(measured=[10,100,3000,15000], dpi=1)
...
for i, name in enumerate(var_names):
    ...
    # Save
    plot_path = f"output/plots/1dcut_{name}_a3profitnogreen_best_a_{best_a}.png"
    plt.savefig(plot_path, dpi=300)
    plt.close()

    print(f"[SUCCESS] Saved 1D cut for {name} in a3profitnogreen a5 to {plot_path}")
