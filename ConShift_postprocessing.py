...
import ConShift as model  # <-- your Habrok extension model file

# ============================================================
# HARDCODED POSTPROCESSING (NO PICKLE) — ConShift.py
...
z_habrok = np.array([0.49983932, 0.95804731, 0.500067,   0.91350635, 0.99912621], dtype=float)  # <-- REPLACE with Habrok z*
...
# ----------------------------
revenue_habrok_reported = None  # e.g. 1412.5678
...
# Defaults (adjust if your Habrok optimum lives elsewhere):
cut_ranges = {
    "f1": (73.0, 80.0),
    "p": (0.29, 0.33),
    "f2": (75.0, 82.0),
...
if SCALE is None:
    raise RuntimeError("ConShift.py does not expose SCALE. Please ensure SCALE is defined at module level.")
if SCALE.size != 5:
    raise RuntimeError(f"Expected SCALE length 5 in ConShift.py, got {SCALE.size}.")
...
print("\n====================")
print("Hardcoded Habrok solution (ConShift) — no pickle")
...
fig.suptitle(
    f"1D Revenue Cuts Around Hardcoded Habrok Solution — ConShift (Rev {revenue_note}={revenue_star:.4f})",
    fontsize=14
)
plt.tight_layout(rect=[0, 0.05, 1, 0.90])

plot_path = output_dir / (
    "1Dcuts_ConShift_A=5000_M=10_K=50.000_HABROK_HARDCODED_"
